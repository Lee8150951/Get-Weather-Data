...
def cityDate(city: str, data: List[int]):
    ...
    # 储存
    path = f"./export/precipitation/{city}.xlsx"
    ...
    for i in range(row):
        ...
        for year in range(2000, 2021):
            ...
            for month in months:
                ...
                ave = mean(tempo)
                # 乘以月份
                monthDay = monthProcess.monthProcess(month)
                currentData = monthDay * ave
                # 数据处理
                factoryData = factory(currentData)
